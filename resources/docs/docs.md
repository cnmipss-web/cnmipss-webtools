...
The `home-routes` handler in the `certification-db.routes.home` namespace
...
The `home-page` function will in turn call the `certification-db.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/certification_db/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `certification-db.handler` namespace:
...
Request middleware functions are located under the `certification-db.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `certification-db.dev-middleware` namespace found in
